...
// FUNCÃO ACTUALIZAR
	...
	function atualizar_doenca($Doenca, $id_sintoma, $informacoes,$Hospital,$Causas,$Prevencao, $id){
	$pdo = conecta();
		try{
		 $editar = $pdo->prepare("UPDATE doencas SET Doenca=?, sintomas_id=?, Descricao=?, markers_id=?,Prevencao=?,Causas=? WHERE id=?");
	     ...
	     $editar->bindValue(4, $Hospital, PDO::PARAM_STR);
         $editar->bindValue(5, $Causas, PDO::PARAM_STR);
         $editar->bindValue(6, $Prevencao, PDO::PARAM_STR);
