...
class relatorio {
   ...
   echo json_encode($dados);
    ...
    public function Nova_Busca($Sintoma){
        $dad='';
        $dadosdoenca =("select Doenca,Descricao,sintomas.primeiro_socorro from doencas join sintomas on doencas.sintomas_id=sintomas.Sintoma  where sintomas_id= '{$Sintoma}'");
    ...
    public function Nova_Causas($Sintoma){
        $dad='';
        $dadosdoenca =("select Causas from doencas join sintomas on doencas.sintomas_id=sintomas.Sintoma  where sintomas_id= '{$Sintoma}'");
        $query = $this->db->query($dadosdoenca);
        $result = $query->fetchAll(PDO::FETCH_OBJ);
        foreach($result as $dados){
            $dad[$dados->Causas] = $dados->Causas;
        }
        if($dad):
            echo json_encode($dad);
        else:
     ...
     public function Nova_prev($Sintoma){
        $dad='';
        $dadosdoenca =("select Prevencao from doencas join sintomas on doencas.sintomas_id=sintomas.Sintoma  where sintomas_id= '{$Sintoma}'");
        $query = $this->db->query($dadosdoenca);
        $result = $query->fetchAll(PDO::FETCH_OBJ);
        foreach($result as $dados){
            $dad[$dados->Prevencao] = $dados->Prevencao;
        }
        if($dad):
            echo json_encode($dad);
        else:
    ...
    // hodp
     public function Nova_Hosp($Sintoma){
        $dad='';
        $dadosdoenca =("select markers_id from doencas join sintomas on doencas.sintomas_id=sintomas.Sintoma  where sintomas_id= '{$Sintoma}'");
        $query = $this->db->query($dadosdoenca);
        $result = $query->fetchAll(PDO::FETCH_OBJ);
        foreach($result as $dados){
            $dad[$dados->markers_id] = $dados->markers_id;
        }
        if($dad):
            echo json_encode($dad);
        else:
     ...
     public function Nova_endd($Sintoma){
        $dad='';
        $dadosdoenca =("select markers_id from doencas join sintomas on doencas.sintomas_id=sintomas.Sintoma  where sintomas_id= '{$Sintoma}'");
        $query = $this->db->query($dadosdoenca);
        $result = $query->fetchAll(PDO::FETCH_OBJ);
